...
from src.modules import RemiEmbedding, RemiHead, TransformerMultiMemoryDecoder, nucleus_sample

class EncoderMultiMemoryDecoderPerformer(pl.LightningModule):
    def __init__(self, config):
        ...
        self.decoder = TransformerMultiMemoryDecoder(config['decoder'])
    ...
    def configure_optimizers(self):
        ...
        if 'max_epochs' in self.config:
            sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=self.config['max_epochs'], eta_min=1e-8)
            return [opt], [sch]
        return opt
    ...
    def forward_mlm(self, trg_inst, inputs):
        ...
        loss = None
        if 'masked_labels' in inputs[trg_inst]:
    ...
    def generate(self, trg_inst, seq, window=1, top_p=1., t=1., device='cuda'):
        ...
        with torch.no_grad():
            for i in tqdm(range(n_bars)):
                ...
                for inst in tracks:
                    src = list(filter(
                        lambda x: not x.startswith('NoteInstFamily'), 
                        MusicRepr.concatenate(tracks[inst][s:i+1]).to_remi(ret='token')
                    ))
                    src = [self.config['const'].all_tokens.index(e) for e in src]
                    inputs[inst] = {'X' : torch.tensor(src).long().to(device).unsqueeze(0), 'X_len' : None}

                res_bar = [0]
                while True:
                    inputs[trg_inst] = {'X' : torch.tensor(res_bar).long().to(device). unsqueeze(0), 'X_len' : None}
        ...
        res = [self.config['const'].all_tokens[e] for e in res]
        i = 0
        while i < len(res):
            if res[i].startswith('NotePitch'):
                res = res[:i] + [f'NoteInstFamily_{trg_inst}'] + res[i:]
                i += 1
            i += 1
        return ' '.join(res)
